...
[Here](https://docs.redhat.com/en/documentation/red_hat_advanced_developer_suite_-_software_supply_chain/1.7/html/customizing_red_hat_advanced_developer_suite_-_software_supply_chain/customizing-sample-software-templates_default) is the reference document.
...
2. **Fork pipelines repository** (Optional):
   ...
   - **Option B**: Manually fork and set up:
     * [TSSC sample pipelines](https://github.com/redhat-appstudio/tssc-sample-pipelines)
...
Fork the repository, synchronize with upstream, and set up your working branch:
...
# Optional, Create and checkout a new branch for customization
git checkout -b customize-templates

# Add upstream remote for syncing
git remote add upstream https://github.com/redhat-appstudio/tssc-sample-templates.git
...
2. **Forks repositories** (if `FORK_REPOS=true`): Automatically forks `tssc-sample-pipelines` to your GitHub organization/user and creates a timestamped branch
3. **Updates Tekton pipeline references** (if `UPDATE_PIPELINE_REF=true`): Updates pipeline URLs in `.tekton/` directories
4. **Updates namespace** (if `DEFAULT_APP_NAMESPACE` is set): Updates namespace in `properties` file and regenerates templates
5. **Commits and pushes changes** (unless `--dry-run`): Automatically stages, commits, and pushes all changes

## Using Your Customized Templates

Your forked sample templates repository is now ready with customized pipeline references to use in Developer Hub.

### Getting Template URLs from Your Git Provider

To use your customized templates in Developer Hub, you need to get the appropriate URL from your forked repository:

1. **Go to your forked sample templates repository** on your Git provider.
...
2. **Get the appropriate URL:**
   ...
   - Example: `https://github.com/username/tssc-sample-templates/blob/main/all.yaml`

3. **Use the URL in Developer Hub** to import your customized templates.
