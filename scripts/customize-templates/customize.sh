...
# Git repository information (set by get_git_info() and used across functions)
CURRENT_GIT_ORG=""
CURRENT_GIT_REPO=""
CURRENT_GIT_BRANCH=""
...
create_branch_from_upstream() {
    ...
    if [[ "$dry_run" == "true" ]]; then
        echo "  [DRY RUN] Would create branch: $branch_name from upstream/$UPSTREAM_PIPELINES_BRANCH"
        return 0
    fi
    
    log "INFO" "Creating branch $branch_name from upstream/$UPSTREAM_PIPELINES_BRANCH in $fork_org/$repo_name..."
    
    # Use the configured upstream branch
    local upstream_branch="$UPSTREAM_PIPELINES_BRANCH"
...
fork_repository() {
    ...
    if check_is_organization "$target_org"; then
        # It's an organization - include organization field
        fork_data="{\"organization\":\"$target_org\"}"
        log "INFO" "Target is an organization, forking to organization account"
    else
        # It's a user account - don't include organization field
        # GitHub API will automatically fork to the authenticated user's account
        fork_data="{}"
...
get_git_info() {
    ...
    # Extract organization and repository from URL using reusable function
...
verify_is_repo_fork() {
    ...
    # Verify organization is not redhat-appstudio
...
run_automated_git_workflow() {
    ...
    echo "Output of 'git status' command:"
    git status --short
    ...
    log "INFO" "Pushing changes to branch ${CURRENT_GIT_BRANCH:-main}..."
...
run_fork_repos() {
    ...
    local branch_name="customize_${branch_suffix}"
    ...
    if [[ "$DRY_RUN" == "false" ]]; then
        # Retry checking if repo exists (fork might take a moment)
        local retry_count=0
        while [[ $retry_count -lt 5 ]] && ! check_repo_exists "$CURRENT_GIT_ORG" "$upstream_repo"; do
    ...
    if ! create_branch_from_upstream "$upstream_org" "$fork_org" "$upstream_repo" "$branch_name" "$DRY_RUN"; then
    ...
    # This runs even in dry-run mode so customize.sh can continue with expected values
    echo -e "\n--- Updating Properties File ---\n"
    sed -i "s|^export PIPELINE__REPO__URL=.*|export PIPELINE__REPO__URL=https://github.com/$fork_org/tssc-sample-pipelines|" "$PROPERTIES_FILE"
...
update_app_namespace_in_templates() {
    ...
    echo "Running update-templates script..."
...
check_prerequisites() {
    ...
    echo "Check 1: GitHub token (GITHUB_TOKEN) is set"
    if ! check_github_variables; then
        prerequisites_passed=false
    fi

    echo "Check 2: Repository is not upstream (redhat-appstudio)"
    if ! verify_is_repo_fork; then
        prerequisites_passed=false
    fi
    
    echo "Check 3: No pre-existing changes in target files"
    ...
    if [[ "$prerequisites_passed" == "true" ]]; then
        ...
        echo "Please fix the issues above before running the customization"
        return 1
    fi
}
...
usage() {
    ...
    echo "  FORK_REPOS              Automatically fork repositories (default: true)"
...
main() {
    ...
    if ! check_prerequisites; then
        log "ERROR" "Prerequisites checks failed, exiting"
        exit 1
    fi
